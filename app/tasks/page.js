    ...
    <div className="page-wrap">
      ...
      <div style={{padding:'1.5rem'}}>
        <div style={{display:'flex',alignItems:'center',justifyContent:'space-between',marginBottom:'0.75rem'}}>
          <div>
            <p style={{fontSize:'0.6rem',fontWeight:500,letterSpacing:'0.28em',textTransform:'uppercase',color:'#873632'}}>Checklist</p>
            <h1 style={{fontFamily:'var(--font-cormorant)',fontSize:'1.8rem',fontWeight:400,color:'#62191C'}}>Tasks</h1>
          </div>
        ...
        <div style={{background:'#fff',border:'1px solid #E0CFC2',padding:'0.9rem 1.1rem',marginBottom:'1rem'}}>
          ...
          return (
            <div key={cat} style={{marginBottom:'1.25rem'}}>
              <div style={{display:'flex',justifyContent:'space-between',marginBottom:6}}>
                <span style={{fontSize:'0.62rem',fontWeight:600,letterSpacing:'0.18em',textTransform:'uppercase',color:'#873632'}}>{cat}</span>
                <span style={{fontSize:'0.65rem',color:'rgba(98,25,28,0.5)'}}>{catDone}/{items.length}</span>
              </div>
              <div style={{background:'#fff',border:'1px solid #E0CFC2',padding:'0.4rem 1rem'}}>
                {items.map((t, i) => (
                  <div key={t.id} style={{display:'flex',alignItems:'center',gap:'0.75rem',padding:'0.6rem 0',borderBottom:i<items.length-1?'1px solid #E0CFC2':'none'}}>
                    <input type="checkbox" checked={t.done==='true'} onChange={()=>toggle(t)} style={{width:16,height:16,accentColor:'#873632',flexShrink:0,cursor:'pointer'}}/>
                    <span style={{flex:1,fontSize:'0.82rem',color:t.done==='true'?'rgba(98,25,28,0.4)':'#62191C',textDecoration:t.done==='true'?'line-through':'none',lineHeight:1.4}}>{t.text}</span>
        ...
        {showForm && (
          <div style={{position:'fixed',inset:0,background:'rgba(98,25,28,0.4)',zIndex:200,display:'flex',alignItems:'flex-end',justifyContent:'center'}} onClick={e=>{if(e.target===e.currentTarget)setShowForm(false)}}>
            <div style={{background:'#fff',width:'100%',maxWidth:480,padding:'1.5rem',borderRadius:'2px 2px 0 0'}}>
              <h3 style={{fontFamily:'var(--font-cormorant)',fontSize:'1.4rem',color:'#62191C',marginBottom:'1.2rem'}}>Add Task</h3>
              <div style={{marginBottom:'0.9rem'}}><label style={S.lbl}>Description *</label><input style={S.inp} value={form.text} onChange={e=>setForm(f=>({...f,text:e.target.value}))} placeholder="What needs to be done?"/></div>
              ...
              <div style={{display:'flex',gap:'0.75rem'}}>
                ...
                <button onClick={addTask} disabled={saving} style={{flex:2,padding:'0.7rem',background:'#62191C',border:'none',color:'#fff',fontSize:'0.72rem',cursor:'pointer',opacity:saving?0.7:1}}>{saving?'Saving…':'Add task'}</button>
              </div>
            </div>
          </div>
        )}
