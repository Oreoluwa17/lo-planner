    ...
    <div className="page-wrap">
      ...
      <div style={{padding:'1.5rem'}}>
        ...
        <div style={{background:'#fff',border:'1px solid #E0CFC2',padding:'1rem 1.25rem',marginBottom:'1rem'}}>
          ...
          <input type="number" value={totalBudget} onChange={e=>setTotalBudget(e.target.value)} placeholder="Enter your total wedding budget" style={{...S.inp,fontSize:'1.1rem',fontWeight:500}}/>
          {tb>0 && (
        ...
        {showForm && (
          <div style={{position:'fixed',inset:0,background:'rgba(98,25,28,0.4)',zIndex:200,display:'flex',alignItems:'flex-end',justifyContent:'center'}} onClick={e=>{if(e.target===e.currentTarget)setShowForm(false)}}>
            <div style={{background:'#fff',width:'100%',maxWidth:480,padding:'1.5rem',borderRadius:'2px 2px 0 0'}}>
              ...
              <div style={{marginBottom:'0.9rem'}}><label style={S.lbl}>Description *</label><input style={S.inp} value={form.description} onChange={e=>setForm(f=>({...f,description:e.target.value}))} placeholder="e.g. Stationery printing"/></div>
              <div style={{display:'grid',gridTemplateColumns:'1fr 1fr',gap:'0.75rem',marginBottom:'0.9rem'}}>
                <div><label style={S.lbl}>Category</label><input style={S.inp} value={form.cat} onChange={e=>setForm(f=>({...f,cat:e.target.value}))} placeholder="e.g. Stationery"/></div>
