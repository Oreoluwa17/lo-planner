...
export default function Nav() {
  ...
  return (
    <>
      <aside className="nav-desktop" style={{ position:'fixed',top:0,left:0,bottom:0,width:220,background:'#62191C',display:'flex',flexDirection:'column',zIndex:50,overflowY:'auto' }}>
        ...
        <div style={{ padding:'1rem 1.25rem',borderBottom:'1px solid rgba(255,255,255,0.08)' }}>
          <div style={{ display:'flex',alignItems:'center',gap:'0.6rem' }}>
            <div style={{ width:32,height:32,borderRadius:'50%',background:'#873632',display:'flex',alignItems:'center',justifyContent:'center',fontSize:'0.8rem',fontWeight:600,color:'#fff',flexShrink:0 }}>{user?.name?.[0]||'·'}</div>
            <div>
              <div style={{ fontSize:'0.82rem',fontWeight:500,color:'#fff' }}>{user?.name||'···'}</div>
              <div style={{ fontSize:'0.62rem',color:'rgba(255,255,255,0.45)',marginTop:1 }}>Signed in</div>
            </div>
          </div>
        </div>
        <nav style={{ padding:'0.75rem 0',flex:1 }}>
          {LINKS.map(l=>{
            ...
            return (
              <Link key={l.href} href={l.href} style={{ display:'flex',alignItems:'center',gap:'0.7rem',padding:'0.65rem 1.25rem',textDecoration:'none',background:active?'rgba(255,255,255,0.12)':'transparent',borderLeft:active?'3px solid #E0CFC2':'3px solid transparent',transition:'background 0.15s' }}>
        ...
        <div style={{ padding:'1rem 1.25rem',borderTop:'1px solid rgba(255,255,255,0.08)' }}>
          <button onClick={signOut} style={{ fontFamily:'var(--font-jost)',fontSize:'0.72rem',fontWeight:500,letterSpacing:'0.12em',textTransform:'uppercase',color:'rgba(255,255,255,0.45)',background:'none',border:'none',cursor:'pointer',padding:0 }}>Sign out</button>
        </div>
      </aside>
        ...
        {LINKS.map(l=>{
          ...
          return (
            <Link key={l.href} href={l.href} style={{ flex:1,display:'flex',flexDirection:'column',alignItems:'center',justifyContent:'center',padding:'0.5rem 0.1rem',textDecoration:'none',gap:2,borderTop:active?'2px solid #E0CFC2':'2px solid transparent' }}>
              <span style={{ fontSize:'0.85rem',lineHeight:1 }}>{l.icon}</span>
              <span style={{ fontSize:'0.36rem',fontWeight:500,letterSpacing:'0.05em',color:active?'#fff':'rgba(255,255,255,0.5)',textTransform:'uppercase' }}>{l.label}</span>
            </Link>
          )
        })}
      </nav>
      ...
      <style>{`
        .nav-desktop{display:flex!important;}.nav-mobile{display:none!important;}.page-wrap{margin-left:220px;}
        @media(max-width:767px){.nav-desktop{display:none!important;}.nav-mobile{display:flex!important;}.page-wrap{margin-left:0;padding-bottom:5.5rem;}}
      `}</style>
    </>
  )
}
