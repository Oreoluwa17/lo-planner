...
const TOTAL_BUDGET = 400000 // Fixed at R400,000
...
export default function Budget() {
  ...
  const totalQuoted = vendors.reduce((s, v) => s + toZAR(v.quote, v.currency || 'ZAR'), 0)
  const totalPaid   = vendors.reduce((s, v) => s + toZAR(v.deposit, v.currency || 'ZAR'), 0)
  const remaining   = TOTAL_BUDGET - totalQuoted
  const pct         = Math.min(100, Math.round(totalQuoted / TOTAL_BUDGET * 100))

  const byDay = { traditional:0, church:0, general:0 }
  vendors.forEach(v => { if (byDay[v.day] !== undefined) byDay[v.day] += toZAR(v.quote, v.currency || 'ZAR') })
  ...
  const SUMMARY = [
    ['Total budget',  fmt(TOTAL_BUDGET),       '#62191C'],
    ['Total quoted',  fmt(totalQuoted),         '#873632'],
    ['Total paid',    fmt(totalPaid),           '#2d6a4f'],
    ['Outstanding',   fmt(totalQuoted-totalPaid),'#9E7161'],
    ['Remaining',     fmt(remaining),           remaining<0?'#a03030':'#62191C'],
  ]
  ...
  return (
    <div className="page-wrap">
      ...
      <div style={{ padding:'1.5rem' }}>
        ...
        {/* Fixed budget banner */}
        <div style={{ background:'#62191C',padding:'1rem 1.25rem',marginBottom:'1rem',display:'flex',alignItems:'center',justifyContent:'space-between' }}>
          <div>
            <div style={{ fontSize:'0.6rem',fontWeight:500,letterSpacing:'0.18em',textTransform:'uppercase',color:'rgba(255,255,255,0.6)',marginBottom:3 }}>Total Wedding Budget</div>
            <div style={{ fontFamily:'var(--font-cormorant)',fontSize:'2rem',fontWeight:300,color:'#fff' }}>R 400,000</div>
          </div>
          <div style={{ textAlign:'right' }}>
            <div style={{ fontSize:'0.6rem',fontWeight:500,letterSpacing:'0.12em',textTransform:'uppercase',color:'rgba(255,255,255,0.6)',marginBottom:3 }}>Used</div>
            <div style={{ fontFamily:'var(--font-cormorant)',fontSize:'2rem',fontWeight:300,color:pct>90?'#ff9999':'#E0CFC2' }}>{pct}%</div>
          </div>
        </div>

        {/* Progress bar */}
        <div style={{ background:'#fff',border:'1px solid #E0CFC2',padding:'0.9rem 1.25rem',marginBottom:'1rem' }}>
          <div style={{ display:'flex',justifyContent:'space-between',fontSize:'0.72rem',color:'#873632',marginBottom:5 }}>
            <span>Quoted: {fmt(totalQuoted)}</span>
            <span>Remaining: {fmt(remaining)}</span>
          </div>
          <div style={{ height:8,background:'#E0CFC2',borderRadius:4 }}>
            <div style={{ height:'100%',background:pct>90?'#a03030':'#873632',borderRadius:4,width:`${pct}%`,transition:'width 0.3s' }}/>
          </div>
          <div style={{ fontSize:'0.65rem',color:'rgba(98,25,28,0.45)',marginTop:4 }}>All amounts converted to ZAR at live exchange rates</div>
        </div>
        ...
        <div style={{ display:'grid',gridTemplateColumns:'repeat(auto-fit,minmax(140px,1fr))',gap:'0.6rem',marginBottom:'1rem' }}>
          {SUMMARY.map(([l,v,c]) => (
            <div key={l} style={{ background:'#fff',border:'1px solid #E0CFC2',padding:'0.8rem 0.9rem' }}>
              ...
              <div style={{ fontFamily:'var(--font-cormorant)',fontSize:'1.3rem',fontWeight:400,color:c }}>{v}</div>
            </div>
          ))}
        </div>
        ...
        <div style={{ background:'#fff',border:'1px solid #E0CFC2',padding:'1rem 1.25rem',marginBottom:'1rem' }}>
          ...
          {Object.entries(byDay).map(([day,amt]) => {
            const dp = TOTAL_BUDGET ? Math.round(amt/TOTAL_BUDGET*100) : 0
            return (
        ...
        {/* Vendor list */}
        <div style={{ background:'#fff',border:'1px solid #E0CFC2',padding:'1rem 1.25rem',marginBottom:'1rem' }}>
          <div style={{ fontSize:'0.75rem',fontWeight:500,color:'#62191C',marginBottom:'0.75rem' }}>Vendor quotes</div>
          {vendors.length===0 ? <p style={{ fontSize:'0.8rem',color:'rgba(98,25,28,0.4)' }}>No vendors added yet.</p>
          : vendors.map(v => {
            const cur = v.zurancy || v.currency || 'ZAR'
            const zarAmt = toZAR(v.quote, cur)
            return (
              <div key={v.id} style={{ display:'flex',alignItems:'center',justifyContent:'space-between',padding:'0.5rem 0',borderBottom:'1px solid #E0CFC2' }}>
                ...
                <div style={{ textAlign:'right' }}>
                  <div style={{ fontSize:'0.82rem',fontWeight:500,color:'#62191C' }}>{fmt(zarAmt)}</div>
                  {cur !== 'ZAR' && v.quote && <div style={{ fontSize:'0.65rem',color:'rgba(98,25,28,0.45)' }}>({cur} {v.quote})</div>}
                  <div style={{ fontSize:'0.68rem',color:'#2d6a4f' }}>Paid: {fmt(toZAR(v.deposit, cur))}</div>
                </div>
              </div>
            )
          })}
        ...
        {showForm && (
          <div style={{ position:'fixed',inset:0,background:'rgba(98,25,28,0.4)',zIndex:200,display:'flex',alignItems:'flex-end',justifyContent:'center' }} onClick={e=>{if(e.target===e.currentTarget)setShowForm(false)}}>
            <div style={{ background:'#fff',width:'100%',maxWidth:480,padding:'1.5rem',borderRadius:'2px 2px 0 0' }}>
              ...
              <div style={{ marginBottom:'0.9rem' }}><label style={S.lbl}>Description *</label><input style={S.inp} value={form.description} onChange={e=>setForm(f=>({...f,description:e.target.value}))} placeholder="e.g. Marriage licence"/></div>
              ...
              <div style={{ display:'flex',gap:'0.75rem' }}>
                ...
                <button onClick={addItem} disabled={saving} style={{ flex:2,padding:'0.7rem',background:'#62191C',border:'none',color:'#fff',fontSize:'0.72rem',cursor:'pointer',opacity:saving?0.7:1 }}>{saving?'Saving…':'Add item'}</button>
              </div>
            </div>
          </div>
        )}
