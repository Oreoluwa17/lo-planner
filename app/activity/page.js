  ...
  return (
    <div className="page-wrap">
      ...
      <div style={{ padding:'1.5rem' }}>
        ...
        <h1 style={{ fontFamily:'var(--font-cormorant)', fontSize:'1.8rem', fontWeight:400, color:C.maroon, marginBottom:'1.25rem' }}>Activity</h1>

        <div style={{ display:'flex', gap:'6px', marginBottom:'1.1rem', flexWrap:'wrap' }}>
          {['all',...users].map(u => (
            <button key={u} onClick={()=>setFilter(u)} style={{ padding:'4px 14px', fontSize:'0.62rem', fontWeight:500, letterSpacing:'0.1em', textTransform:'uppercase', border:'1px solid '+C.blush, background:filter===u?C.maroon:C.white, color:filter===u?'#fff':C.rust, cursor:'pointer', borderRadius:20 }}>
              {u==='all'?'Everyone':u}
            </button>
          ))}
        </div>
