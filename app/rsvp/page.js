...
export default function RSVP() {
    ...
    fetch('/api/rsvp')
      ...
      .then(d => {
        if (d.error) {
          ...
          // Guard: make sure data is always an array
          const list = Array.isArray(d.data) ? d.data : []
          setGuests(list)
        }
      })
      .catch(err => setError('Failed to load RSVP list. ' + err.message))
      .finally(() => setLoading(false))
  }, [])

  // Safe array operations with fallbacks
  const safeGuests   = Array.isArray(guests) ? guests : []
  const accepting    = safeGuests.filter(g => g.attending === 'Joyfully accepts')
  const declining    = safeGuests.filter(g => g.attending === 'Regretfully declines')
  const totalGuests  = accepting.reduce((s, g) => s + (Number(g.guests) || 1), 0)

  const filtered = safeGuests
    .filter(g => {
      if (filter === 'attending') return g.attending === 'Joyfully accepts'
      if (filter === 'declining') return g.attending === 'Regretfully declines'
      return true
    })
    .filter(g => {
  ...
  function fmt(ts) {
    if (!ts) return ''
    try { return new Date(ts).toLocaleDateString('en-ZA', { day:'numeric', month:'short', year:'numeric' }) }
    catch { return '' }
  }
    ...
    <div className="page-wrap">
      ...
      <div style={{ padding:'1.5rem' }}>
        ...
        <div style={{ display:'grid',gridTemplateColumns:'repeat(3,1fr)',gap:'0.6rem',marginBottom:'1.1rem' }}>
          {[
            ['Total RSVPs',  safeGuests.length, '#62191C'],
        ...
        {accepting.length > 0 && (
          <div style={{ background:'#62191C',padding:'0.8rem 1.1rem',marginBottom:'1.1rem',display:'flex',justifyContent:'space-between',alignItems:'center' }}>
            <span style={{ fontSize:'0.72rem',fontWeight:500,letterSpacing:'0.1em',textTransform:'uppercase',color:'rgba(255,255,255,0.7)' }}>Total headcount (incl. +1s)</span>
        ...
        {loading ? (
          <p style={{ fontSize:'0.85rem',color:'rgba(98,25,28,0.45)' }}>Loading…</p>
        ) : error ? (
          <div style={{ background:'#fff',border:'1px solid #E0CFC2',padding:'1.5rem',textAlign:'center' }}>
            ...
            <p style={{ fontSize:'0.75rem',color:'rgba(98,25,28,0.5)',lineHeight:1.6 }}>
              Make sure <strong>RSVP_SCRIPT_URL</strong> is set in Vercel environment variables and the Apps Script has been redeployed as a new version.
            </p>
          </div>
        ) : filtered.length === 0 ? (
          <p style={{ fontSize:'0.85rem',color:'rgba(98,25,28,0.4)',textAlign:'center',padding:'2rem' }}>
            {safeGuests.length === 0 ? 'No RSVPs yet.' : 'No guests match your search.'}
          </p>
        ) : (
                ...
                <div key={i} style={{ display:'flex',alignItems:'flex-start',gap:'0.85rem',padding:'0.9rem 1.1rem',borderBottom:i<filtered.length-1?'1px solid #E0CFC2':'none' }}>
                  ...
                  <div style={{ flex:1,minWidth:0 }}>
                    <div style={{ display:'flex',alignItems:'center',gap:8,flexWrap:'wrap' }}>
                      ...
                      {attending && Number(g.guests) > 1 && (
                        <span style={{ fontSize:'0.6rem',color:'rgba(98,25,28,0.5)' }}>+{Number(g.guests)-1} guest{Number(g.guests)>2?'s':''}</span>
                      )}
                    </div>
                    <div style={{ fontSize:'0.72rem',color:'rgba(98,25,28,0.55)',marginTop:2 }}>
                      {g.email}{g.phone ? ' · '+g.phone : ''}
                    </div>
                    {g.message && (
                      <div style={{ fontSize:'0.72rem',color:'rgba(98,25,28,0.6)',marginTop:4,fontStyle:'italic' }}>"{g.message}"</div>
                    )}
